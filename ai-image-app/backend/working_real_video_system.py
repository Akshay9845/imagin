#!/usr/bin/env python3
"""
Working Real Video Generation System
...
# Import diffusers at module level to avoid scope issues
try:
    from diffusers import DiffusionPipeline, StableDiffusionPipeline, StableVideoDiffusionPipeline
...
class WorkingRealVideoSystem:
    def __init__(self, output_dir="working_real_video_outputs"):
        """Initialize the working real video generation system"""
        ...
        if DIFFUSERS_AVAILABLE:
            ...
            logger.error("Diffusers library not available - cannot load models")
    
    def load_models(self):
        """Load real video generation models with updated libraries"""
        logger.info("Loading working real video generation models...")
        
        try:
            # Load text-to-video models
...
def main():
    """Test the working real video generation system"""
    system = WorkingRealVideoSystem()
    
    print("🎬 Working Real Video Generation System")
    print("=" * 50)
    
    # Show system info
    info = system.get_system_info()
    print(f"Device: {info['device']}")
    print(f"Diffusers available: {'✅' if info['diffusers_available'] else '❌'}")
