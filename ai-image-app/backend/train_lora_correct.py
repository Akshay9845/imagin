#!/usr/bin/env python3
"""
Correct LAION-2B-en LoRA training script with proper Linear layer targeting
"""
...
class CorrectLAIONTrainer:
    def __init__(self, 
        ...
        self.lora_success = self.setup_lora_correct()
        
        # Create output directories
        os.makedirs("lora_weights", exist_ok=True)
        os.makedirs("training_logs", exist_ok=True)
        
    def find_linear_modules(self):
        """Find all Linear modules in the UNet that can be targeted by LoRA"""
        logger.info("Finding Linear modules for LoRA targeting...")
        
        linear_modules = []
        
        # Walk through all named modules and find Linear layers
        ...
        # Filter for attention-related Linear modules
        attention_linear = []
        for name in linear_modules:
            if any(keyword in name.lower() for keyword in ['to_q', 'to_k', 'to_v', 'to_out', 'proj']):
                attention_linear.append(name)
        
        logger.info(f"Found {len(attention_linear)} attention-related Linear modules")
        if attention_linear:
            logger.info(f"Sample attention Linear modules: {attention_linear[:10]}")
        
        return linear_modules, attention_linear
    
    def setup_lora_correct(self):
        """Setup LoRA with correct Linear layer targeting"""
        linear_modules, attention_linear = self.find_linear_modules()
        
        if not attention_linear:
            logger.error("No attention Linear modules found!")
            return False
        
        try:
            # Use attention Linear modules for LoRA
            logger.info("Setting up LoRA with attention Linear modules...")
            
            lora_config = LoraConfig(
                r=16,
                lora_alpha=32,
                target_modules=attention_linear[:20],  # Use first 20 attention modules
                lora_dropout=0.1,
                bias="none",
                task_type="CAUSAL_LM"
            )
            ...
            # Try with fewer modules
            try:
                logger.info("Trying with fewer attention modules...")
                
                lora_config = LoraConfig(
                    r=8,
                    lora_alpha=16,
                    target_modules=attention_linear[:10],  # Use first 10
                    lora_dropout=0.1,
                    bias="none",
                    task_type="CAUSAL_LM"
                )
                ...
                logger.error(f"Reduced LoRA setup also failed: {e2}")
                return False
    ...
    def train_streaming_round(self, batch_size=100, seed=None):
        """Complete training round on LAION-2B-en with proper LoRA training"""
...
def main():
    ...
    print("🚀 Correct LAION-2B-en LoRA Training")
    print("=" * 50)
    
    trainer = CorrectLAIONTrainer(
        batch_size=1,
        learning_rate=1e-4,
