#!/usr/bin/env python3
"""
Compatible LAION-2B-en LoRA training script using Stable Diffusion v1.5
"""
...
class CompatibleLAIONTrainer:
    def __init__(self, 
                 base_model="runwayml/stable-diffusion-v1-5",  # Use SD v1.5 for better LoRA compatibility
                 batch_size=1,
                 learning_rate=1e-4,
                 max_train_steps=100):
        ...
        # Load base model (SD v1.5 is more compatible with PEFT LoRA)
        ...
        # Setup LoRA with compatible configuration
        self.lora_success = self.setup_lora_compatible()
        
        # Create output directories
        os.makedirs("lora_weights", exist_ok=True)
        os.makedirs("training_logs", exist_ok=True)
        
    def setup_lora_compatible(self):
        """Setup LoRA with compatible configuration for SD v1.5"""
        try:
            logger.info("Setting up LoRA with compatible configuration for SD v1.5...")
            
            # Use SD v1.5 compatible LoRA config
            ...
            logger.error(f"Compatible LoRA setup failed: {e}")
            
            # Try with different target modules
            try:
                logger.info("Trying with different target modules...")
    ...
    def train_streaming_round(self, batch_size=100, seed=None):
        """Complete training round on LAION-2B-en with compatible LoRA training"""
...
def main():
    ...
    print("🚀 Compatible LAION-2B-en LoRA Training (SD v1.5)")
    print("=" * 50)
    
    trainer = CompatibleLAIONTrainer(
        batch_size=1,
        learning_rate=1e-4,
