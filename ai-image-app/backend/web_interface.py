#!/usr/bin/env python3
"""
Simple web interface for the ultra-realistic generation system
"""
...
<body>
    ...
    <script>
        ...
        document.getElementById('image-form').addEventListener('submit', async (e) => {
            ...
            showStatus('image-status', '🎨 Generating ultra-realistic image... This may take a few minutes.', 'loading');
            ...
            try {
                ...
                showStatus('image-status', `❌ Network error: ${error.message}`, 'error');
            } finally {
        ...
        document.getElementById('video-form').addEventListener('submit', async (e) => {
            ...
            showStatus('video-status', '🎬 Generating ultra-realistic video... This may take several minutes.', 'loading');
            document.getElementById('video-btn').disabled = true;
            
            try {
                const response = await fetch('/api/ultra-realistic/generate-video', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (result.success) {
                    showStatus('video-status', '✅ Video generated successfully!', 'success');
                    document.getElementById('video-result').innerHTML = `
                        <h3>Generated Video:</h3>
                        <video controls style="max-width: 100%;">
                            <source src="/api/ultra-realistic/video/${result.filename}" type="video/mp4">
                    ...
                    showStatus('video-status', `❌ Error: ${result.error}`, 'error');
                }
            } catch (error) {
                showStatus('video-status', `❌ Network error: ${error.message}`, 'error');
            } finally {
...
def api_generate_video():
    ...
    try:
        ...
        video_path = system.generate_ultra_realistic_video(
            prompt=data['prompt'],
            style=data.get('style', 'photorealistic'),
            duration_seconds=int(data.get('duration', 5)),
...
if __name__ == '__main__':
    ...
    print("🌐 Starting Ultra-Realistic Web Interface...")
