#!/usr/bin/env python3
"""
Fixed LoRA Training Script for Mac M3
Uses correct task type for Stable Diffusion compatibility
"""
...
class FixedLoRATrainer:
    ...
    def load_models(self):
        """Load base models and setup LoRA with correct task type"""
        ...
        # Setup LoRA configuration with correct task type
        lora_config = LoraConfig(
            ...
            task_type=TaskType.CAUSAL_LM,  # Fixed: Use CAUSAL_LM for SD compatibility
        )
    ...
    def save_checkpoint(self, step: int, loss: float):
        ...
        # Save LoRA weights
        checkpoint_path = checkpoint_dir / f"lora_step_{step}_fixed"
    ...
    def save_final_model(self):
        ...
        # Save LoRA weights
        self.unet.save_pretrained(final_path / "lora_model_fixed")
        
        # Save training config
        with open(final_path / "training_config_fixed.json", 'w') as f:
    ...
    def log_training_info(self, step: int, loss: float, success: bool):
        ...
        log_file = log_dir / f"training_info_fixed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
